...
void write_file ( FILE* file, int size, char* orig ) {
	for ( int j = 0; j < size; j++ ) {
		fprintf( file, "%c", orig[j] );
	}
}
...
int decode_base64 ( FILE* in_file, FILE* out_file ) {
	...
	while ( EOF != read ) {
		...
		if ( '=' != read) {
				base64[size] = strchr( ALPHA, read ) - ALPHA;
				size++;
			}	
		if ( INPUT_SIZE == size ) {
			decode( orig, base64 );
			write_file( out_file, OUTPUT_SIZE, orig );
			size = 0;
	...
	if ( 0 != size ) {
		decode( orig, base64 );
		write_file( out_file, size - 1, orig );
	}
	return 0;
}
